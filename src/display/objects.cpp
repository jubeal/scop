...
void Object::compute()
{
  ...
  int uvsEmpty = UVS.empty();
  int jump = uvsEmpty ? 3 : 5;
  ...
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, jump * sizeof(float), (void *)0);
  glEnableVertexAttribArray(0);
  if (!UVS.empty())
  {
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, jump * sizeof(float), (void *)(3 * sizeof(float)));
...
void Object::draw(Shader currentShader)
{
  ...
  currentShader.setVector4("color", COLOR);
